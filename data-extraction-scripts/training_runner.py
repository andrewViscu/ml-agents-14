...
def run_training(
    ...
    try:
        ...
        written_steps = set()  # Track steps to avoid duplicates
        ...
        for line in process.stdout:
            ...
            # If we have a step and at least one metric,
            # and haven't written this step yet, write to CSV
            if current_step is not None and current_step not in written_steps:
                if (
                    current_mean_reward is not None
                    or current_std_reward is not None
                    or current_mean_group_reward is not None
                ):

                    # Calculate performance and resource usage data
                    # (called on each line with metrics)
                    resource_usage_data = (
                        resource_monitor.calculate_resource_usage_data()
                    )
                    ...
                    row_data = {
                        ...
                        "mean_group_reward": (
                            current_mean_group_reward
                            if current_mean_group_reward is not None
                            else ""
                        ),
                        "std_reward": (
                            current_std_reward
                            if current_std_reward is not None
                            else ""
                        ),
                        "time_elapsed": (
                            current_time_elapsed
                            if current_time_elapsed is not None
                            else ""
                        ),
        ...
        resource_monitor.join(timeout=2.0)  # Wait thread up to 2s
        ...
        print(
            "\nError: 'mlagents-learn' command not found. "
            "Make sure ML-Agents is installed and in your PATH."
        )
        raise
