...
def run_reward_prediction(df, threshold=0.05):
    ...
    mean_score, scores = cross_validation(
        X, y, k=5, model_class=MyRandomForest, n_trees=50
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='ML-Agents Training Data Predictor'
        )
    parser.add_argument(
    ...
    parser.add_argument(
        '--files', type=str, nargs='+', help='Specific files to load'
        )
    parser.add_argument(
        '--mode',
        type=str,
        default='all',
        choices=[
            "reward", "algorithm", "environment", "convergence", "all"
        ],
        help='Which analysis to run'
                        )
    parser.add_argument(
        '--threshold', type=float, default=0.05,
        help='Reward threshold for classification'
        )
    ...
    if df is None or len(df) == 0:
        ...
        print(f"Make sure CSV files are in {args.data_dir}"
              f"or specify files with --files")
        return
